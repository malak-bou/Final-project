...
from schemas import (
    ...
    CourseMaterial as CourseMaterialSchema, CourseUpdate, RoleChangeResponse
)
from auth import (
    verify_password,
    get_password_hash,
...
@app.get("/users/role-change-requests", response_model=List[RoleChangeResponse])
def get_role_change_requests(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    if current_user.type_utilisateur != UserType.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can view role change requests"
        )
    ...
    return [
        RoleChangeResponse(
            message=f"Pending role change request for user {user.id}",
            new_role=user.requested_role
        )
        for user in users
    ]

@app.post("/users/{user_id}/reject-role-change", response_model=RoleChangeResponse)
def reject_role_change(
    ...
    if current_user.type_utilisateur != UserType.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can reject role changes"
        )
    ...
    if not user.role_change_requested:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No role change request pending for this user"
        )
...
@app.get("/departments/{department_id}/users", response_model=List[schemas.User])
